...
| <a id="Q10"></a>Q10 | Resolve `eksctl`'s ambient AWS profile like Q7 did for `aws` | `security` | 💤 | S | `eval_eksctl` shares the AWS profile/region fallback but keeps its combined `ask_ambient`. Reuse `aws_default_profile()` so a prod default profile denies an unpinned mutating `eksctl`. |
